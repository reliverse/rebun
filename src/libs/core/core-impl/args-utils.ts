...
export type PartialMatcherOptions = {
  dot?: boolean;
  nocase?: boolean;
};
...
export function getPartialMatcher(
  ...
  // the following pattern is faster than using `.push()`
  ...
  // @ts-expect-error [ts(2322)] Type '(input: string) => boolean' is not assignable to type 'Matcher'. Type 'boolean' is not assignable to type 'Result'.
  return (input: string) => {
